...
1. Nhập đường dẫn **Input Folder** (mặc định `INPUT`, chứa 3 thư mục con `facebook/`, `vat/`, `bank/` — tool tự đọc nội dung PDF để phân loại, **không cần** người dùng sắp xếp đúng thư mục) và **Output Folder**.
...
File PDF gốc **không bao giờ** bị sửa hay xoá — mọi thao tác trong OUTPUT chỉ là copy.
